...
var require_disposable = __commonJS({
  "node_modules/vscode-jsonrpc/lib/common/disposable.js"(exports) {
    ...
    var Disposable14;
    (function(Disposable15) {
      function create(func) {
        return {
          dispose: func
        };
      }
      Disposable15.create = create;
    })(Disposable14 = exports.Disposable || (exports.Disposable = {}));
  }
});
...
var require_events = __commonJS({
  "node_modules/vscode-jsonrpc/lib/common/events.js"(exports) {
    ...
    exports.Emitter = Emitter9;
    Emitter9._noop = function() {
    };
  }
});
...
var require_main = __commonJS({
  "node_modules/vscode-jsonrpc/lib/node/main.js"(exports) {
    ...
    function generateRandomPipeName() {
      ...
      let result;
      if (XDG_RUNTIME_DIR) {
        result = path18.join(XDG_RUNTIME_DIR, `vscode-ipc-${randomSuffix}.sock`);
      } else {
        result = path18.join(os4.tmpdir(), `vscode-${randomSuffix}.sock`);
      }
...
var language, Range3, Position2, Location2, TextEdit2, WorkspaceEdit2, SymbolKind3;
  ...
  "src/server/utils/typeConverters.ts"() {
    ...
    ((WorkspaceEdit8) => {
      ...
      function withFileCodeEdits(workspaceEdit, client, edits) {
        var _a, _b;
        let changes = (_a = workspaceEdit.changes) != null ? _a : {};
        for (const edit of edits) {
          const resource = client.toResource(edit.fileName);
          let arr = (_b = changes[resource]) != null ? _b : [];
...
var require_windows = __commonJS({
  "node_modules/isexe/windows.js"(exports, module2) {
    ...
    function isexe(path18, options, cb) {
      fs10.stat(path18, function(er, stat) {
        cb(er, er ? false : checkStat(stat, path18, options));
      });
    }
    function sync(path18, options) {
      return checkStat(fs10.statSync(path18), path18, options);
    }
  }
});
...
var require_mode = __commonJS({
  "node_modules/isexe/mode.js"(exports, module2) {
    ...
    function sync(path18, options) {
      return checkStat(fs10.statSync(path18), options);
    }
...
var import_coc45, import_fs7, import_path11, import_util, nulToken, updateImportsOnFileMoveName, UpdateImportsOnFileRenameHandler;
    ...
    UpdateImportsOnFileRenameHandler = class {
      ...
      async flushRenames() {
        ...
        for (const group of this.groupRenames(renames)) {
          ...
          if (Object.keys((_a = edits.changes) != null ? _a : {}).length > 0) {
      ...
      async promptUser(newResources) {
        ...
        const response = await import_coc45.window.showInformationMessage(
          newResources.length === 1 ? `Update imports for '${import_path11.default.basename(newResources[0].fsPath)}'?` : this.getConfirmMessage(`Update imports for the following ${newResources.length} files?`, newResources),
          rejectItem,
          acceptItem
        );
        switch (response) {
      ...
      getConfirmMessage(start, resourcesToConfirm) {
        ...
        const paths = [start];
        paths.push("");
        paths.push(...resourcesToConfirm.slice(0, MAX_CONFIRM_FILES).map((r) => import_path11.default.basename(r.fsPath)));
...
async function installModules(uri, names) {
  ...
  if (!root || !import_fs.default.existsSync(import_path.default.join(root, "package.json"))) {
    import_coc.window.showErrorMessage(`package.json not found from workspaceFolder: ${root}`);
    return;
  }
  ...
  try {
    ...
    statusItem.dispose();
    import_coc.window.showErrorMessage(`Install error ${e.message}`);
    return;
  }
  statusItem.dispose();
  import_coc.window.showInformationMessage(`Installed: ${exists.join(" ")}`);
}
...
function runCommand2(cmd, opts = {}, timeout) {
  if (process.platform !== "win32") {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (timeout) {
      timer = setTimeout(() => {
        cp.kill("SIGKILL");
        reject(new import_coc2.CancellationError());
      }, timeout * 1e3);
    }
    cp = (0, import_child_process2.exec)(cmd, opts, (err, stdout, stderr) => {
      if (timer)
        clearTimeout(timer);
      if (err) {
        reject(new Error(`exited with ${err.code}
...
var Resolver = class {
  get nodeFolder() {
    ...
    if (!cmd)
      return Promise.resolve("");
    return runCommand2(`${cmd} --loglevel silent root -g`, {}, 3e3).then((root) => {
  ...
  get yarnFolder() {
    ...
    if (!cmd)
      return Promise.resolve("");
    return runCommand2(`${cmd} global dir`, {}, 3e3).then((root) => {
      let folder = import_path2.default.join(stripAnsi(root).trim(), "node_modules");
      let exists = import_fs2.default.existsSync(folder);
      if (exists)
  ...
  resolve(folder) {
    let file2 = import_path2.default.join(folder, moduleName, "lib/tsserver.js");
    if (import_fs2.default.existsSync(file2))
      return file2;
    return void 0;
  }
};
...
var ChooseVersionCommand = class {
  ...
  async execute() {
    ...
    if (!res || res.version.path == currPath)
      return;
    if (res.version.path == (localVersion == null ? void 0 : localVersion.path)) {
...
var OrganizeImportsCommand = class {
  ...
  async _execute(client, document, sortOnly = false) {
    ...
    const response = await client.interruptGetErr(() => client.execute("organizeImports", args, import_coc4.CancellationToken.None));
    ...
    if (keys.length == 1) {
      let doc = import_coc4.workspace.getDocument(keys[0]);
      if (doc) {
    ...
    if (edit)
      await import_coc4.workspace.applyEdit(edit);
  }
...
var OrganizeImportsCodeActionProvider = class {
  constructor(client, fileConfigManager) {
    ...
    this.metadata = {
      providedCodeActionKinds: [import_coc4.CodeActionKind.SourceOrganizeImports]
    };
  }
  async provideCodeActions(document, _range, context, token) {
    if (this.client.modeIds.indexOf(document.languageId) == -1)
      return;
    if (!context.only || !context.only.includes(import_coc4.CodeActionKind.SourceOrganizeImports)) {
      return [];
    }
    ...
    const organizeImportsAction = import_vscode_languageserver_protocol.CodeAction.create("Organize Imports", {
      ...
      command: "tsserver.organizeImports",
      arguments: [document]
    }, import_coc4.CodeActionKind.SourceOrganizeImports);
...
var _WatchProject = class {
  constructor(service, logger) {
    ...
    task.onExit((code) => {
      if (code != 0) {
        import_coc5.window.showWarningMessage(`TSC exit with code ${code}`);
      }
      this.onStop();
    });
    task.onStdout((lines) => {
    ...
    task.onStderr((lines) => {
      import_coc5.window.showErrorMessage(`TSC error: ` + lines.join("\n"));
    });
  ...
  onLine(line) {
    if (countRegex.test(line)) {
      ...
      if (!ms)
        return;
      let fullpath = import_path3.default.join(this.options.cwd, ms[1]);
  ...
  async getOptions() {
    let doc = await import_coc5.workspace.document;
    let resource = import_coc5.Uri.parse(doc.uri);
    const client = await this.service.getClientHost();
    let { serviceClient } = client;
    const file2 = serviceClient.toPath(resource.toString());
    if (!file2) {
      import_coc5.window.showWarningMessage("Could not determine TypeScript or JavaScript project. Current file should be javascript or typescript file.");
      return;
    }
    let version;
    const rootPath = serviceClient.getWorkspaceRootForResource(resource);
    if (!rootPath) {
      import_coc5.window.showWarningMessage(`Could not determine workspace folder for current file ${doc.uri}.`);
      return;
    }
    ...
    if (!version) {
      import_coc5.window.showErrorMessage(`Unable to resolve typescript module from workspace folders`);
      return;
    }
    let tsconfigPath;
    let configFileName = await resolveConfigFile(serviceClient, file2);
    if (configFileName) {
      tsconfigPath = import_path3.default.relative(rootPath, configFileName);
    } else {
      this.logger.warn(`Unable to resolve config file from ${file2}, try configuration`);
      let inspect = import_coc5.workspace.getConfiguration("tsserver", doc.uri).inspect("tsconfigPath");
      let value = inspect ? inspect.workspaceFolderValue || inspect.workspaceValue || "tsconfig.json" : "tsconfig.json";
      let fullpath = import_path3.default.join(rootPath, value);
      if (import_fs3.default.existsSync(fullpath)) {
        tsconfigPath = value;
      } else {
        import_coc5.window.showWarningMessage(`Config file ${fullpath} not exists.`);
        return;
      }
    }
    return {
...
async function resolveConfigFile(client, file2) {
  let res;
  try {
    res = await client.execute("projectInfo", { file: file2, needFileNameList: false }, import_coc5.CancellationToken.None);
  } catch {
  }
...
var FileConfigurationManager = class {
  constructor(client) {
    ...
    this.disposables = [];
    import_coc6.workspace.onDidCloseTextDocument((textDocument) => {
  ...
  getFormattingOptions(document) {
    const editor = import_coc6.window.visibleTextEditors.find((editor2) => editor2.document.uri === document.uri);
    return editor ? {
  ...
  async ensureConfigurationForDocument(document, token) {
    ...
    if (cached) {
      ...
      opts = await import_coc6.workspace.getFormatOptions(document.uri);
    }
  ...
  getLanguageConfiguration(languageId) {
    return import_coc6.workspace.getConfiguration(languageId);
  }
  ...
  formatEnabled(document) {
    ...
    const config = import_coc6.workspace.getConfiguration(`${language2}.format`, document);
  ...
  getFormatOptions(options, language2, uri) {
    const config = import_coc6.workspace.getConfiguration(`${language2}.format`, uri);
    return {
  ...
  getPreferences(language2, doc) {
    ...
    let { uri } = doc;
    const config = import_coc6.workspace.getConfiguration(language2, doc);
    const preferencesConfig = import_coc6.workspace.getConfiguration(`${language2}.preferences`, doc);
    const preferences = {
      ...
      autoImportFileExcludePatterns: this.getAutoImportFileExcludePatternsPreference(preferencesConfig, (_a = import_coc6.workspace.getWorkspaceFolder(uri)) == null ? void 0 : _a.uri),
  ...
  getAutoImportFileExcludePatternsPreference(config, workspaceFolder) {
    var _a;
    let folder = workspaceFolder ? import_coc6.Uri.parse(workspaceFolder).fsPath : import_coc6.workspace.root;
    return workspaceFolder && ((_a = config.get("autoImportFileExcludePatterns")) == null ? void 0 : _a.map((p) => {
      const slashNormalized = p.replace(/\\/g, "/");
      const isRelative = /^\.\.?($|\/)/.test(slashNormalized);
      return import_path4.default.isAbsolute(p) ? p : p.startsWith("*") ? "/" + slashNormalized : isRelative ? import_path4.default.join(folder, p) : "/**/" + slashNormalized;
    }));
  }
  dispose() {
...
function getSymbolKind(item) {
  switch (item.kind) {
    case "method":
      return import_vscode_languageserver_protocol2.SymbolKind.Method;
    case "enum":
      return import_vscode_languageserver_protocol2.SymbolKind.Enum;
    case "function":
      return import_vscode_languageserver_protocol2.SymbolKind.Function;
    case "class":
      return import_vscode_languageserver_protocol2.SymbolKind.Class;
    case "interface":
      return import_vscode_languageserver_protocol2.SymbolKind.Interface;
    case "var":
      return import_vscode_languageserver_protocol2.SymbolKind.Variable;
    default:
      return import_vscode_languageserver_protocol2.SymbolKind.Variable;
  }
}
var TypeScriptWorkspaceSymbolProvider = class {
  ...
  async provideWorkspaceSymbols(search, token) {
    ...
    for (const item of response.body) {
      ...
      const symbolInfo = import_vscode_languageserver_protocol2.SymbolInformation.create(
        label,
        getSymbolKind(item),
        range,
      ...
      if (kindModifiers == null ? void 0 : kindModifiers.has(KindModifiers.deprecated)) {
        symbolInfo.tags = [import_vscode_languageserver_protocol2.SymbolTag.Deprecated];
      }
  ...
  getUri() {
    const documents = import_coc7.workspace.textDocuments;
  ...
  get searchAllOpenProjects() {
    return this.client.apiVersion.gte(API.v390) && import_coc7.workspace.getConfiguration("typescript").get("workspaceSymbols.scope", "allOpenProjects") === "allOpenProjects";
  }
};
...
var TypeScriptBaseCodeLensProvider = class {
  constructor(client, cachedResponse, modeId) {
    ...
    this.modeId = modeId;
    this.onDidChangeCodeLensesEmitter = new import_vscode_languageserver_protocol3.Emitter();
  }
...
function fromProtocolCallHierarchyItem(item) {
  ...
  const name = useFileName ? import_path5.default.basename(item.file) : item.name;
  const detail = useFileName ? import_path5.default.relative(import_coc8.workspace.cwd, import_path5.default.dirname(item.file)) : (_a = item.containerName) != null ? _a : "";
  const result = {
    name,
    detail,
    uri: import_coc8.Uri.file(item.file).toString(),
  ...
  if (kindModifiers == null ? void 0 : kindModifiers.has(KindModifiers.deprecated)) {
    result.tags = [import_vscode_languageserver_protocol4.SymbolTag.Deprecated];
  }
  return result;
}
...
async function applyCodeAction(client, action) {
  ...
  if (workspaceEdit) {
    if (!await import_coc9.workspace.applyEdit(workspaceEdit)) {
      return false;
    }
  }
...
async function applyCodeActionCommands(client, action) {
  if (action.commands && action.commands.length) {
    for (const command of action.commands) {
      const response = await client.execute("applyCodeActionCommand", { command }, import_coc9.CancellationToken.None);
...
function convertCompletionEntry(tsEntry, uri, position, context) {
  ...
  let insertTextFormat = context.enableCallCompletions && (kind === import_coc10.CompletionItemKind.Function || kind === import_coc10.CompletionItemKind.Method) ? import_coc10.InsertTextFormat.Snippet : import_coc10.InsertTextFormat.PlainText;
  ...
  if (tsEntry.replacementSpan) {
    ...
    if (start.line == end.line) {
      textEdit = {
        range: import_coc10.Range.create(start.line - 1, start.offset - 1, end.line - 1, end.offset - 1),
  ...
  if (tsEntry.kindModifiers) {
    ...
    if (kindModifiers.has(KindModifiers.deprecated)) {
      tags = [import_vscode_languageserver_protocol5.CompletionItemTag.Deprecated];
    }
    if (kindModifiers.has(KindModifiers.color)) {
      kind = import_coc10.CompletionItemKind.Color;
    }
...
function convertKind(kind) {
  switch (kind) {
    ...
    case Kind.keyword:
      return import_coc10.CompletionItemKind.Keyword;
    case Kind.const:
      return import_coc10.CompletionItemKind.Constant;
    case Kind.let:
    case Kind.variable:
    case Kind.localVariable:
    case Kind.alias:
      return import_coc10.CompletionItemKind.Variable;
    case Kind.memberVariable:
    case Kind.memberGetAccessor:
    case Kind.memberSetAccessor:
      return import_coc10.CompletionItemKind.Field;
    case Kind.function:
    case Kind.localFunction:
      return import_coc10.CompletionItemKind.Function;
    case Kind.method:
    case Kind.constructSignature:
    case Kind.callSignature:
    case Kind.indexSignature:
      return import_coc10.CompletionItemKind.Method;
    case Kind.enum:
      return import_coc10.CompletionItemKind.Enum;
    case Kind.module:
    case Kind.externalModuleName:
      return import_coc10.CompletionItemKind.Module;
    case Kind.class:
    case Kind.type:
      return import_coc10.CompletionItemKind.Class;
    case Kind.interface:
      return import_coc10.CompletionItemKind.Interface;
    case Kind.warning:
    case Kind.script:
      return import_coc10.CompletionItemKind.File;
    case Kind.directory:
      return import_coc10.CompletionItemKind.Folder;
  }
  return import_coc10.CompletionItemKind.Variable;
}
...
function toResource(filepath) {
  return import_coc11.Uri.file(filepath);
}
...
var _ApplyCompletionCodeActionCommand = class {
  ...
  async execute(codeActions) {
    ...
    const idx = await import_coc12.window.showQuickpick(codeActions.map((o) => o.description), "Select code action to apply");
    if (idx < 0)
      return;
...
((CompletionConfiguration2) => {
  ...
  function getConfigurationForResource(modeId, resource) {
    const config = import_coc12.workspace.getConfiguration(modeId, resource);
    return {
...
var TypeScriptCompletionItemProvider = class {
  constructor(client, typingsStatus, fileConfigurationManager, lang) {
    ...
    this.lang = lang;
    this.currentLine = "";
    import_coc12.commands.registerCommand(ApplyCompletionCodeActionCommand.ID, async (codeActions) => {
    ...
    if (this.client.apiVersion.gte(API.v300)) {
      try {
        ...
        if (isMemberCompletion) {
          ...
          if (dotMatch) {
            const range = import_vscode_languageserver_protocol7.Range.create({
              line: position.line,
  ...
  async resolveCompletionItem(item, token) {
    ...
    if (!item.data.isSnippet && detail && item.insertTextFormat == import_vscode_languageserver_protocol7.InsertTextFormat.Snippet) {
...
`;
  ...
  createSnippetOfFunctionCall(item, detail) {
    ...
    snippet.appendText(`${(_a = item.insertText) != null ? _a : item.label}(`);
  ...
  isInValidCommitCharacterContext(document, position) {
    if (this.client.apiVersion.lt(API.v320)) {
      if (position.character > 1) {
        const preText = document.getText(import_vscode_languageserver_protocol7.Range.create(
          position.line,
          0,
          position.line,
          position.character
...
function convertDocumentHighlight(highlight) {
  return highlight.highlightSpans.map((span) => {
    return {
      ...
      kind: span.kind === "writtenReference" ? import_vscode_languageserver_protocol8.DocumentHighlightKind.Write : import_vscode_languageserver_protocol8.DocumentHighlightKind.Read
    };
  });
}
...
var getSymbolKind2 = (kind) => {
  switch (kind) {
    case Kind.module:
      return import_vscode_languageserver_protocol9.SymbolKind.Module;
    case Kind.class:
      return import_vscode_languageserver_protocol9.SymbolKind.Class;
    case Kind.enum:
      return import_vscode_languageserver_protocol9.SymbolKind.Enum;
    case Kind.interface:
      return import_vscode_languageserver_protocol9.SymbolKind.Interface;
    case Kind.method:
      return import_vscode_languageserver_protocol9.SymbolKind.Method;
    case Kind.memberVariable:
      return import_vscode_languageserver_protocol9.SymbolKind.Property;
    case Kind.memberGetAccessor:
      return import_vscode_languageserver_protocol9.SymbolKind.Property;
    case Kind.memberSetAccessor:
      return import_vscode_languageserver_protocol9.SymbolKind.Property;
    case Kind.variable:
      return import_vscode_languageserver_protocol9.SymbolKind.Variable;
    case Kind.const:
      return import_vscode_languageserver_protocol9.SymbolKind.Variable;
    case Kind.localVariable:
      return import_vscode_languageserver_protocol9.SymbolKind.Variable;
    ...
    case Kind.constructSignature:
    case Kind.constructorImplementation:
      return import_vscode_languageserver_protocol9.SymbolKind.Constructor;
    case Kind.function:
    case Kind.localFunction:
      return import_vscode_languageserver_protocol9.SymbolKind.Function;
  }
  return import_vscode_languageserver_protocol9.SymbolKind.Variable;
};
var TypeScriptDocumentSymbolProvider = class {
  ...
  static convertSymbol(item, range) {
    ...
    const symbolInfo = import_vscode_languageserver_protocol9.DocumentSymbol.create(
      label,
      "",
    ...
    if (kindModifiers.has(KindModifiers.deprecated)) {
      symbolInfo.tags = [import_vscode_languageserver_protocol9.SymbolTag.Deprecated];
    }
    return symbolInfo;
  }
...
var SourceFixAll = class extends SourceAction {
  constructor() {
    super();
    this.kind = import_coc15.CodeActionKind.SourceFixAll;
...
SourceFixAll.kind = import_coc15.CodeActionKind.SourceFixAll;
var SourceRemoveUnused = class extends SourceAction {
  constructor() {
    super();
    this.kind = import_coc15.CodeActionKind.Source;
...
SourceRemoveUnused.kind = import_coc15.CodeActionKind.Source;
var SourceAddMissingImports = class extends SourceAction {
  constructor() {
    super();
    this.kind = import_coc15.CodeActionKind.Source;
...
SourceAddMissingImports.kind = import_coc15.CodeActionKind.Source;
var _TypeScriptAutoFixProvider = class {
  ...
  async provideCodeActions(document, _range, context, token) {
    if (!context.only || !context.only.some((s) => s === import_coc15.CodeActionKind.Source)) {
      return void 0;
    }
  ...
  async provideCodeActions(document, _range, context, _token) {
    ...
    for (const diagnostic of diagnostics) {
      ...
      if (nodeModules.indexOf(name) !== -1) {
        names.push(name);
        edits.push({
          range: import_vscode_languageserver_protocol10.Range.create(0, 0, 0, 0),
          newText: `import ${name} from '${name}'
`
        });
        command = "tsserver.organizeImports";
...
function createCompleteItem(document, position) {
  var _a;
  const line = (_a = document.lines[position.line]) != null ? _a : "";
...
var InstallModuleProvider = class {
  ...
  async provideCodeActions(document, _range, context, _token) {
    ...
    for (let name of names) {
      ...
      let codeAction = import_vscode_languageserver_protocol11.CodeAction.create(title, command, import_vscode_languageserver_protocol11.CodeActionKind.QuickFix);
...
var _ApplyFixAllCodeAction = class {
  ...
  async execute(document, file2, tsAction) {
    ...
    await this.formattingConfigurationManager.ensureConfigurationForDocument(document, import_vscode_languageserver_protocol12.CancellationToken.None);
    const args = {
      scope: {
        type: "file",
        args: { file: file2 }
    ...
    try {
      const res = await this.client.execute("getCombinedCodeFix", args, import_vscode_languageserver_protocol12.CancellationToken.None);
      if (res.type != "response") {
        return;
      }
      let { body } = res;
      ...
      const token = import_vscode_languageserver_protocol12.CancellationToken.None;
      const { commands: commands6 } = body;
      if (commands6 && commands6.length) {
...
var SupportedCodeActionProvider = class {
  ...
  get supportedCodeActions() {
    if (!this._supportedCodeActions) {
      return new Promise((resolve, reject) => {
        this.client.execute("getSupportedCodeFixes", null, import_vscode_languageserver_protocol12.CancellationToken.None).then((res) => {
          if (res.type !== "response") {
...
var TypeScriptQuickFixProvider = class {
  constructor(client, formattingConfigurationManager, diagnosticsManager) {
    this.client = client;
    this.formattingConfigurationManager = formattingConfigurationManager;
    this.diagnosticsManager = diagnosticsManager;
  ...
  getSingleFixForTsCodeAction(diagnostic, tsAction) {
    const codeAction = {
      ...
      kind: import_vscode_languageserver_protocol12.CodeActionKind.QuickFix
    };
...
var _ApplyRefactoringCommand = class {
  ...
  async execute(document, file2, refactor, action, range) {
    ...
    const response = await this.client.execute("getEditsForRefactor", args, import_vscode_languageserver_protocol13.CancellationToken.None);
...
var _TypeScriptRefactorProvider = class {
  ...
  async provideCodeActions(document, range, context, token) {
    ...
    return this.convertApplicableRefactors(
      ...
      file2,
      range,
      context.only && context.only.some((v) => v.includes(import_vscode_languageserver_protocol13.CodeActionKind.Refactor))
    ).filter((action) => {
    ...
    for (const info of body) {
      if (info.inlineable === false) {
        const codeAction = {
          ...
          kind: import_vscode_languageserver_protocol13.CodeActionKind.Refactor
        };
        codeAction.command = {
          title: info.description,
  ...
  shouldTrigger(context) {
    if (context.only && context.only.every((o) => !o.includes(import_vscode_languageserver_protocol13.CodeActionKind.Refactor))) {
      return false;
    }
    return true;
  }
  static getKind(refactor) {
    ...
    return import_vscode_languageserver_protocol13.CodeActionKind.Refactor;
  }
...
TypeScriptRefactorProvider.extractFunctionKind = import_vscode_languageserver_protocol13.CodeActionKind.RefactorExtract + ".function";
TypeScriptRefactorProvider.extractConstantKind = import_vscode_languageserver_protocol13.CodeActionKind.RefactorExtract + ".constant";
TypeScriptRefactorProvider.moveKind = import_vscode_languageserver_protocol13.CodeActionKind.Refactor + ".move";
TypeScriptRefactorProvider.metadata = {
  providedCodeActionKinds: [import_vscode_languageserver_protocol13.CodeActionKind.Refactor]
};
...
var TypeScriptRenameProvider = class {
  ...
  async renameFile(fileToRename, newName, token) {
    if (!import_path6.default.extname(newName)) {
      newName += import_path6.default.extname(fileToRename);
    }
    const dirname = import_path6.default.dirname(fileToRename);
    const newFilePath = import_path6.default.join(dirname, newName);
    const args = {
      file: fileToRename,
  ...
  async provideSemanticTokens(document, requestArg, token) {
    ...
    for (let i = 0; i < tokenSpan.length; ) {
      ...
      for (let line = startPos.line; line <= endPos.line; line++) {
        ...
        const endCharacter = line === endPos.line ? endPos.character : ((_a = document.lines[line]) != null ? _a : "").length;
...
var LanguageProvider = class {
  constructor(client, fileConfigurationManager, description, typingsStatus) {
    ...
    this.disposables = [];
    import_coc28.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
  ...
  configurationChanged() {
    const config = import_coc28.workspace.getConfiguration(this.id, null);
  ...
  registerProviders(client, typingsStatus) {
    ...
    this._register(
      import_coc28.languages.registerCompletionItemProvider(
        clientId,
        "TSC",
        documentSelector.syntax,
        new TypeScriptCompletionItemProvider(client, typingsStatus, this.fileConfigurationManager, this.description.id),
        TypeScriptCompletionItemProvider.triggerCharacters
      )
    );
    this._register(
      import_coc28.languages.registerCompletionItemProvider(
        `tsc-${this.description.id}-jsdoc`,
        "TSC",
        documentSelector.syntax,
    ...
    if (this.client.apiVersion.gte(API.v230)) {
      this._register(import_coc28.languages.registerCompletionItemProvider(
    ...
    this._register(import_coc28.languages.registerDefinitionProvider(documentSelector.syntax, definitionProvider));
    this._register(import_coc28.languages.registerTypeDefinitionProvider(documentSelector.syntax, definitionProvider));
    if (hasSemantic) {
      this._register(import_coc28.languages.registerImplementationProvider(documentSelector.semantic, definitionProvider));
    }
    this._register(import_coc28.languages.registerReferencesProvider(documentSelector.syntax, new TypeScriptReferences(client)));
    this._register(import_coc28.languages.registerHoverProvider(documentSelector.syntax, new TypeScriptHoverProvider(client)));
    this._register(import_coc28.languages.registerDocumentHighlightProvider(documentSelector.syntax, new TypeScriptDocumentHighlightProvider(this.client)));
    this._register(import_coc28.languages.registerSignatureHelpProvider(documentSelector.syntax, new TypeScriptSignatureHelpProvider(client), ["(", ",", "<", ")"]));
    this._register(import_coc28.languages.registerDocumentSymbolProvider(documentSelector.syntax, new TypeScriptDocumentSymbolProvider(client)));
    if (hasSemantic) {
      this._register(import_coc28.languages.registerRenameProvider(documentSelector.semantic, new TypeScriptRenameProvider(client, this.fileConfigurationManager)));
    }
    let formatProvider = new TypeScriptFormattingProvider(client, this.fileConfigurationManager);
    this._register(import_coc28.languages.registerDocumentFormatProvider(documentSelector.syntax, formatProvider));
    this._register(import_coc28.languages.registerDocumentRangeFormatProvider(documentSelector.syntax, formatProvider));
    this._register(import_coc28.languages.registerOnTypeFormattingEditProvider(documentSelector.syntax, formatProvider, [";", "}", "\n", String.fromCharCode(27)]));
    this._register(import_coc28.languages.registerCodeActionProvider(documentSelector.syntax, new InstallModuleProvider(client), "tsserver"));
    if (this.client.apiVersion.gte(API.v380) && typeof import_coc28.languages["registerCallHierarchyProvider"] === "function" && hasSemantic) {
      this._register(import_coc28.languages.registerCallHierarchyProvider(documentSelector.semantic, new TypeScriptCallHierarchySupport(client)));
    }
    if (this.client.apiVersion.gte(API.v370) && hasSemantic) {
      const provider = new TypeScriptDocumentSemanticTokensProvider(client);
      if (typeof import_coc28.languages["registerDocumentRangeSemanticTokensProvider"] === "function") {
        this._register(import_coc28.languages.registerDocumentRangeSemanticTokensProvider(documentSelector.semantic, provider, provider.getLegend()));
      }
    }
    ...
    if (this.client.apiVersion.gte(API.v280)) {
      this._register(import_coc28.languages.registerFoldingRangeProvider(documentSelector.syntax, new TypeScriptFoldingProvider(this.client)));
      if (hasSemantic) {
        this._register(
          import_coc28.languages.registerCodeActionProvider(
            documentSelector.semantic,
            new OrganizeImportsCodeActionProvider(this.client, this.fileConfigurationManager),
            "tsserver",
            [import_coc28.CodeActionKind.SourceOrganizeImports]
          )
        );
      }
    }
    if (this.client.apiVersion.gte(API.v240) && hasSemantic) {
      this._register(
        import_coc28.languages.registerCodeActionProvider(
          documentSelector.semantic,
          new TypeScriptRefactorProvider(client, this.fileConfigurationManager),
          "tsserver",
          [import_coc28.CodeActionKind.Refactor]
        )
      );
    }
    if (this.client.apiVersion.gte(API.v300) && hasSemantic) {
      this._register(
        import_coc28.languages.registerCodeActionProvider(
          documentSelector.semantic,
          new TypeScriptAutoFixProvider(client, this.fileConfigurationManager, client.diagnosticsManager),
          "tsserver",
          [import_coc28.CodeActionKind.Source, import_coc28.CodeActionKind.SourceFixAll]
        )
      );
    }
    if (hasSemantic) {
      this._register(
        import_coc28.languages.registerCodeActionProvider(
          documentSelector.semantic,
          new TypeScriptQuickFixProvider(client, this.fileConfigurationManager, client.diagnosticsManager),
          "tsserver",
          [import_coc28.CodeActionKind.QuickFix]
        )
      );
    }
    this._register(
    ...
    if (hasSemantic) {
      ...
      if (this.client.apiVersion.gte(API.v206) && conf.get("referencesCodeLens.enabled")) {
        this._register(import_coc28.languages.registerCodeLensProvider(documentSelector.semantic, new TypeScriptReferencesCodeLensProvider(client, cachedResponse, this.description.id)));
      }
      if (this.client.apiVersion.gte(API.v220) && conf.get("implementationsCodeLens.enabled")) {
        this._register(import_coc28.languages.registerCodeLensProvider(documentSelector.semantic, new TypeScriptImplementationsCodeLensProvider(client, cachedResponse, this.description.id)));
      }
    }
    if (this.client.apiVersion.gte(API.v350)) {
      this._register(import_coc28.languages.registerSelectionRangeProvider(documentSelector.syntax, new SmartSelection(this.client)));
    }
    if (this.client.apiVersion.gte(API.v300)) {
      this._register(new TagClosing(this.client, this.description.id));
    }
    if (this.client.apiVersion.gte(API.v440) && hasSemantic) {
      if (typeof import_coc28.languages.registerInlayHintsProvider === "function") {
        let provider = new TypeScriptInlayHintsProvider(this.description, this.client, this.fileConfigurationManager);
        this._register(provider);
        this._register(import_coc28.languages.registerInlayHintsProvider(documentSelector.semantic, provider));
      } else {
  ...
  handles(resource, doc) {
    ...
    return this.handlesConfigFile(import_coc28.Uri.parse(resource));
  }
  handlesConfigFile(uri) {
    const base2 = import_path7.default.basename(uri.fsPath);
    return !!base2 && (!!this.description.configFilePattern && this.description.configFilePattern.test(base2));
  }
  handlesUri(resource) {
    const ext = import_path7.default.extname(resource.path).slice(1).toLowerCase();
  ...
  diagnosticsReceived(diagnosticsKind, resource, diagnostics) {
    const config = import_coc28.workspace.getConfiguration(this.id, resource);
    ...
    this.client.diagnosticsManager.updateDiagnostics(resource, this._diagnosticLanguage, diagnosticsKind, diagnostics.filter((diag) => {
      if (!reportUnnecessary) {
        if (diag.reportUnnecessary && diag.severity === import_coc28.DiagnosticSeverity.Information) {
          return false;
        }
      }
      if (!reportDeprecated) {
        if (diag.reportDeprecated && diag.severity === import_coc28.DiagnosticSeverity.Hint) {
          return false;
        }
      }
      return true;
...
var getRootTempDir = (() => {
  ...
  return () => {
    if (!dir) {
      dir = path8.join(os.tmpdir(), `coc-tsserver`);
    }
...
var getInstanceTempDir = (() => {
  let dir;
  return () => {
    dir != null ? dir : dir = path8.join(getRootTempDir(), makeRandomHexString(20));
    if (!fs4.existsSync(dir)) {
      fs4.mkdirSync(dir);
    }
    return dir;
...
function getTempFile(prefix) {
  return path8.join(getInstanceTempDir(), `${prefix}-${makeRandomHexString(20)}.tmp`);
}
...
// src/server/tsServer/logDirectoryProvider.ts
var base = path9.join(import_os.default.tmpdir(), "coc-tsserver-log");
var NodeLogDirectoryProvider = class {
  ...
  getNewLogDirectory() {
    ...
    if (root) {
      try {
        return fs6.mkdtempSync(path9.join(root, `tsserver-log-`));
      } catch (e) {
        return void 0;
      }
    }
  ...
  logDirectory() {
    try {
      const path18 = base;
      if (!fs6.existsSync(path18)) {
        fs6.mkdirSync(path18, { recursive: true });
      }
      return path18;
    } catch {
      return void 0;
...
var Disposable6 = class {
  ...
  dispose() {
    ...
    (0, import_coc29.disposeAll)(this._disposables);
  }
  _register(value) {
    if (this._isDisposed) {
      value.dispose();
...
// src/server/tsServer/server.ts
init_typescriptService();
var _ProcessBasedTsServer = class extends Disposable6 {
  constructor(_serverId, _serverSource, _process, _tsServerLogFile, _requestCanceller, _version, _tracer) {
    ...
    this._onEvent = this._register(new import_coc30.Emitter());
    this.onEvent = this._onEvent.event;
    this._onExit = this._register(new import_coc30.Emitter());
    this.onExit = this._onExit.event;
    this._onError = this._register(new import_coc30.Emitter());
...
var _RequestRouter = class {
  ...
  execute(command, args, executeInfo) {
    if (_RequestRouter.sharedCommands.has(command) && typeof executeInfo.executionTarget === "undefined") {
      ...
      let token = void 0;
      if (executeInfo.token) {
        const source = new import_coc30.CancellationTokenSource();
        executeInfo.token.onCancellationRequested(() => {
...
var _GetErrRoutingTsServer = class extends Disposable6 {
  constructor(servers, delegate) {
    super();
    this._onEvent = this._register(new import_coc30.Emitter());
    this.onEvent = this._onEvent.event;
...
var _SyntaxRoutingTsServer = class extends Disposable6 {
...
var Reader = class extends Disposable6 {
  constructor(readable) {
    ...
    this.onError = this._onError.event;
    this._onData = this._register(new import_coc31.Emitter());
...
function generatePatchedEnv(env, modulePath) {
  ...
  newEnv["NODE_PATH"] = path10.join(modulePath, "..", "..", "..");
...
var IpcChildServerProcess = class extends Disposable6 {
...
var StdioChildServerProcess = class extends Disposable6 {
  constructor(_process) {
    super();
    this._process = _process;
...
function defaultPathNormalizer(resource) {
  let u = import_coc32.Uri.parse(resource);
  if (u.scheme === "file") {
    return u.fsPath;
  }
...
var ResourceMap = class {
  ...
  isCaseInsensitivePath(path18) {
    if (isWindowsPath(path18)) {
      return true;
    }
    return path18[0] === "/" && this.onIsCaseInsenitiveFileSystem;
  }
...
function isWindowsPath(path18) {
  return /^[a-zA-Z]:\\/.test(path18);
}
...
var SyncedBuffer = class {
  ...
  open() {
    let folder = import_coc33.workspace.getWorkspaceFolder(this.document.uri);
    const args = {
      file: this.filepath,
      fileContent: this.document.getText(),
      projectRootPath: folder ? import_coc33.Uri.parse(folder.uri).fsPath : void 0
    };
...
var SyncedBufferMap = class extends ResourceMap {
  getForPath(filePath) {
    return this.get(import_coc33.Uri.file(filePath).toString());
  }
  get allBuffers() {
    return this.values;
  }
...
var BufferSyncSupport = class {
  constructor(client, modeIds) {
    ...
    const pathNormalizer = (path18) => this.client.toPath(path18);
    ...
    import_coc33.workspace.onDidChangeConfiguration(this.updateConfiguration, this, this.disposables);
  }
  listen() {
    if (this.listening)
      return;
    this.listening = true;
    import_coc33.workspace.onDidOpenTextDocument(
      this.openTextDocument,
      this,
      this.disposables
    );
    import_coc33.workspace.onDidCloseTextDocument(
      this.onDidCloseTextDocument,
      this,
      this.disposables
    );
    import_coc33.workspace.onDidChangeTextDocument(
      this.onDidChangeTextDocument,
      this,
      this.disposables
    );
    import_coc33.workspace.textDocuments.forEach(this.openTextDocument, this);
  }
  handles(resource) {
  ...
  dispose() {
    ...
    (0, import_coc33.disposeAll)(this.disposables);
  ...
  ensureHasBuffer(resource) {
    ...
    const existingDocument = import_coc33.workspace.textDocuments.find((doc) => doc.uri.toString() === resource);
    if (existingDocument) {
  ...
  toResource(filePath) {
    ...
    return import_coc33.Uri.file(filePath).toString();
  }
  reset() {
    var _a;
  ...
  openTextDocument(document) {
    ...
    const filepath = this.client.normalizedPath(import_coc33.Uri.parse(resource));
    if (!filepath) {
      return false;
    }
    if (this.syncedBuffers.has(resource)) {
  ...
  sendPendingDiagnostics() {
    ...
    if (orderedFileSet.size) {
      let uris = Array.from(orderedFileSet.uris).map((uri) => import_coc33.Uri.parse(uri));
  ...
  updateConfiguration() {
    const jsConfig = import_coc33.workspace.getConfiguration("javascript", null);
    const tsConfig = import_coc33.workspace.getConfiguration("typescript", null);
  ...
  getSuggestionDiagnostics(settings) {
    ...
    return this.get(2 /* Suggestion */).filter((x) => {
      if (!enableSuggestions) {
        return x.tags && (x.tags.includes(import_coc34.DiagnosticTag.Unnecessary) || x.tags.includes(import_coc34.DiagnosticTag.Deprecated));
      }
      return true;
    });
  }
...
var DiagnosticsManager = class extends Disposable6 {
  constructor() {
    super();
    ...
    this._currentDiagnostics = this._register(import_coc34.languages.createDiagnosticCollection("tsserver"));
  }
  dispose() {
    super.dispose();
...
var ImplicitProjectConfiguration = class {
  ...
  static readCheckJs(configuration) {
    var _a;
    return (_a = configuration.get("tsserver.implicitProjectConfig.checkJs")) != null ? _a : configuration.get("tsserver.implicitProjectConfig.checkJs", false);
  }
  static readExperimentalDecorators(configuration) {
    var _a;
    return (_a = configuration.get("tsserver.implicitProjectConfig.experimentalDecorators")) != null ? _a : configuration.get("tsserver.implicitProjectConfig.experimentalDecorators", false);
  }
...
var ServiceConfigurationProvider = class {
  loadFromWorkspace() {
    const configuration = import_coc35.workspace.getConfiguration();
    return {
  ...
  readLocalTsdk(configuration) {
    var _a;
    const inspect = configuration.inspect("tsserver.tsdk");
    let value = (_a = inspect.workspaceValue) != null ? _a : inspect.workspaceFolderValue;
  ...
  fixPathPrefixes(inspectValue) {
    const pathPrefixes = ["~" + import_path8.default.sep];
    for (const pathPrefix of pathPrefixes) {
      if (inspectValue.startsWith(pathPrefix)) {
        return import_path8.default.join(import_os2.default.homedir(), inspectValue.slice(pathPrefix.length));
      }
    }
    return inspectValue;
  }
...
var TypeScriptServerSpawner = class {
  ...
  getTsServerArgs(kind, configuration, currentVersion, apiVersion, pluginManager, cancellationPipeName) {
    ...
    if (TypeScriptServerSpawner.isLoggingEnabled(configuration)) {
      ...
      if (logDir) {
        tsServerLogFile = path12.join(logDir, `tsserver.log`);
...
var TypeScriptVersion = class {
  constructor(path18, _pathLabel) {
    this.path = path18;
    this._pathLabel = _pathLabel;
    this._api = null;
  }
  get tscPath() {
    return import_path9.default.resolve(this.path, "../bin/tsc");
  }
  get tsServerPath() {
    return import_path9.default.resolve(this.path, "tsserver.js");
  }
  get pathLabel() {
  ...
  getTypeScriptVersion(serverPath) {
    ...
    const p = serverPath.split(import_path9.default.sep);
    if (p.length <= 2) {
      return void 0;
    }
    const p2 = p.slice(0, -2);
    const modulePath = p2.join(import_path9.default.sep);
    let fileName = import_path9.default.join(modulePath, "package.json");
    if (!import_fs6.default.existsSync(fileName)) {
      if (import_path9.default.basename(modulePath) === "built") {
        fileName = import_path9.default.join(modulePath, "..", "package.json");
      }
    }
    if (!import_fs6.default.existsSync(fileName)) {
      return void 0;
...
var TypeScriptVersionProvider = class {
  ...
  get globalVersion() {
    let { globalTsdk } = this.configuration;
    let folder = import_coc36.workspace.expand(globalTsdk);
    if (!import_path9.default.isAbsolute(folder)) {
      folder = import_path9.default.join(import_coc36.workspace.root, folder);
    }
    if (globalTsdk)
  ...
  getVersionFromTscPath(tscPath) {
    if (!tscPath || !import_fs6.default.existsSync(tscPath))
      return void 0;
    let libFolder = import_path9.default.resolve(tscPath, "../../lib");
  ...
  getLocalVersionFromFolder(workspaceRoot) {
    for (let folder of MODULE_FOLDERS) {
      let libFolder = import_path9.default.join(workspaceRoot, folder);
      if (import_fs6.default.existsSync(libFolder)) {
        let version = new TypeScriptVersion(libFolder);
        if (version.isValid)
          return version;
  ...
  getLocalVersion() {
    let folders = import_coc36.workspace.workspaceFolders.map((f) => import_coc36.Uri.parse(f.uri).fsPath);
  ...
  get bundledVersion() {
    try {
      ...
      const bundledVersion = new TypeScriptVersion(
        import_path9.default.dirname(file2),
        ""
      );
      return bundledVersion;
    } catch (e) {
      import_coc36.window.showErrorMessage("Bundled typescript module not found");
      return null;
    }
  }
};
...
var Logger2 = class {
  get output() {
    ...
    this._channel = import_coc37.window.createOutputChannel("tsserver");
    return this._channel;
  }
  dispose() {
    if (this._channel) {
...
var RelativeWorkspacePathResolver = class {
  static asAbsoluteWorkspacePath(relativePath) {
    for (const root of import_coc38.workspace.workspaceFolders || []) {
      const rootPrefixes = [`./${root.name}/`, `${root.name}/`, `.\\${root.name}\\`, `${root.name}\\`];
      for (const rootPrefix of rootPrefixes) {
        if (relativePath.startsWith(rootPrefix)) {
          return path14.join(import_coc38.Uri.parse(root.uri).fsPath, relativePath.replace(rootPrefix, ""));
        }
      }
    }
    return void 0;
...
var TypeScriptPluginPathsProvider = class {
  ...
  resolvePluginPath(pluginPath) {
    if (path15.isAbsolute(pluginPath)) {
      return [pluginPath];
    }
    ...
    return (import_coc39.workspace.workspaceFolders || []).map((workspaceFolder) => path15.join(import_coc39.Uri.parse(workspaceFolder.uri).fsPath, pluginPath));
  }
};
...
var Tracer = class {
  ...
  static readTrace() {
    let result = Trace.fromString(import_coc40.workspace.getConfiguration("tsserver").get("trace.server", "off"));
...
var VersionStatus = class {
  constructor() {
    this._versionString = "";
    this._versionBarEntry = import_coc42.window.createStatusBarItem(99);
    this._onChangeEditor = import_coc42.window.onDidChangeActiveTextEditor(this.onChangeEditor, this);
...
var TypeScriptServiceClient = class extends Disposable6 {
  constructor(context, modeIds, services2, tscPathVim) {
    super();
    this.context = context;
    this.modeIds = modeIds;
    this.token = 0;
    this.state = import_coc43.ServiceStat.Initial;
    this.logger = new Logger2();
    this.hasServerFatallyCrashedTooManyTimes = false;
    this._onTsServerStarted = this._register(new import_coc43.Emitter());
    this.onTsServerStarted = this._onTsServerStarted.event;
    this._onDiagnosticsReceived = this._register(new import_coc43.Emitter());
    this.onDiagnosticsReceived = this._onDiagnosticsReceived.event;
    this._onConfigDiagnosticsReceived = this._register(new import_coc43.Emitter());
    this.onConfigDiagnosticsReceived = this._onConfigDiagnosticsReceived.event;
    this._onResendModelsRequested = this._register(new import_coc43.Emitter());
    this.onResendModelsRequested = this._onResendModelsRequested.event;
    this._onProjectLanguageServiceStateChanged = this._register(new import_coc43.Emitter());
    this.onProjectLanguageServiceStateChanged = this._onProjectLanguageServiceStateChanged.event;
    this._onDidBeginInstallTypings = this._register(new import_coc43.Emitter());
    this.onDidBeginInstallTypings = this._onDidBeginInstallTypings.event;
    this._onDidEndInstallTypings = this._register(new import_coc43.Emitter());
    this.onDidEndInstallTypings = this._onDidEndInstallTypings.event;
    this._onTypesInstallerInitializationFailed = this._register(new import_coc43.Emitter());
    this.onTypesInstallerInitializationFailed = this._onTypesInstallerInitializationFailed.event;
    this._onDidChangeCapabilities = this._register(new import_coc43.Emitter());
    ...
    this.pathSeparator = import_path10.default.sep;
    ...
    import_coc43.workspace.onDidChangeConfiguration(() => {
  ...
  restartTsServer(fromUserAction = false) {
    ...
    if (this.serverState.type === 1 /* Running */) {
      ...
      this.state = import_coc43.ServiceStat.Stopping;
      this.serverState.server.kill();
    }
    if (fromUserAction) {
      this.hasServerFatallyCrashedTooManyTimes = false;
  ...
  startService(resendModels = false) {
    ...
    if (!version.isValid) {
      void import_coc43.window.showWarningMessage(`The path ${version.path} doesn't point to a valid tsserver install. Falling back to bundled TypeScript version.`);
    ...
    this.state = import_coc43.ServiceStat.Starting;
    ...
    handle.onError((err) => {
      ...
      if (err) {
        this.state = import_coc43.ServiceStat.StartFailed;
        import_coc43.window.showErrorMessage(`TypeScript language server exited with error. Error message is: ${err.message || err.name}`);
      }
    ...
    handle.onExit((data) => {
      ...
      this.state = import_coc43.ServiceStat.Stopped;
      if (handle.tsServerLogFile) {
    ...
    import_coc43.workspace.nvim.setVar("Coc_tsserver_path", version.tscPath, true);
    this.serviceStarted(resendModels);
    this._onReady.resolve();
    this._onTsServerStarted.fire(version.version);
    this.versionStatus.onDidChangeTypeScriptVersion(version);
    this.state = import_coc43.ServiceStat.Running;
  ...
  async openTsServerLogFile() {
    ...
    let showWarning = (msg) => {
      import_coc43.window.showWarningMessage(msg);
    };
    if (isRoot) {
    ...
    try {
      let uri = import_coc43.Uri.file(tsServerLogFile);
      await import_coc43.workspace.jumpTo(uri.toString(), void 0, "tabe");
      return true;
    } catch {
    ...
    if (restart) {
      ...
      if (this.numberRestarts > 5) {
        ...
        if (diff < 10 * 1e3) {
          ...
          import_coc43.window.showErrorMessage(
...
This may be caused by a plugin contributed by one of these extensions: ${pluginExtensionList}` : "The JS/TS language service immediately crashed 5 times. The service will not be restarted."
          ...
          this.lastStart = Date.now();
          import_coc43.window.showWarningMessage(
...
This may be caused by a plugin contributed by one of these extensions: ${pluginExtensionList}` : "The JS/TS language service crashed 5 times in the last 5 Minutes."
  ...
  toPath(uri) {
    return this.normalizedPath(import_coc43.Uri.parse(uri));
  }
  ...
  toResource(filepath) {
    ...
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc43.Uri.parse(filepath);
        if (this.inMemoryResourcePrefix) {
          const dirName = import_path10.default.dirname(resource.path);
          const fileName = import_path10.default.basename(resource.path);
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
            resource = resource.with({ path: import_path10.default.posix.join(dirName, fileName.slice(this.inMemoryResourcePrefix.length)) });
          }
        }
        return resource.toString();
      }
    }
    return import_coc43.Uri.file(filepath).toString();
  }
  normalizedPath(resource) {
    ...
    switch (resource.scheme) {
      case file: {
        ...
        if (!result)
          return void 0;
        result = import_path10.default.normalize(result);
  ...
  getDocument(resource) {
    if (resource.startsWith("untitled:")) {
      ...
      return import_coc43.workspace.getDocument(bufnr);
    }
    return import_coc43.workspace.getDocument(resource);
  }
  ...
  asUrl(filepath) {
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc43.Uri.parse(filepath.slice(this.inMemoryResourcePrefix.length));
        if (this.inMemoryResourcePrefix) {
          const dirName = import_path10.default.dirname(resource.path);
          const fileName = import_path10.default.basename(resource.path);
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
            resource = resource.with({
              path: import_path10.default.posix.join(
                dirName,
    ...
    return import_coc43.Uri.file(filepath);
  }
  execute(command, args, token, config) {
    ...
    if (config == null ? void 0 : config.cancelOnResourceChange) {
      ...
      if (runningServerState.type === 1 /* Running */) {
        const source = new import_coc43.CancellationTokenSource();
  ...
  fatalError(command, error) {
    ...
    this.error(msg);
    import_coc43.window.showErrorMessage(msg);
  ...
  dispatchEvent(event) {
    ...
    switch (event.event) {
      ...
      case "projectsUpdatedInBackground" /* projectsUpdatedInBackground */: {
        this.versionStatus.loading = false;
        const body = event.body;
        const resources = body.openFiles.map((file2) => import_coc43.Uri.parse(this.toResource(file2)));
  ...
  getWorkspaceRootForResource(resource) {
    const roots = import_coc43.workspace.workspaceFolders ? Array.from(import_coc43.workspace.workspaceFolders) : void 0;
    if (!(roots == null ? void 0 : roots.length))
      return void 0;
    const uris = roots.map((f) => import_coc43.Uri.parse(f.uri));
    switch (resource.scheme) {
      case file:
      case untitled:
      ...
      case memFs:
        for (const uri of uris.sort((a, b) => a.fsPath.length - b.fsPath.length)) {
          if (resource.fsPath.startsWith(uri.fsPath + import_path10.default.sep)) {
            return uri.fsPath;
          }
        }
...
var AtaProgressReporter = class {
  constructor(client) {
    ...
    this._invalid = false;
    this.statusItem = import_coc44.window.createStatusBarItem(10, { progress: true });
    const disposables = [];
  ...
  async onTypesInstallerInitializationFailed() {
    this.statusItem.hide();
    if (!this._invalid) {
      const config = import_coc44.workspace.getConfiguration("typescript");
      if (config.get("check.npmIsInstalled", true)) {
        const dontShowAgain = "Don't Show Again";
        const selected = await import_coc44.window.showWarningMessage(
...
var TypeScriptServiceClientHost = class {
  constructor(descriptions, pluginManager, tscPath, context) {
    ...
    this.disposables.push(import_coc46.languages.registerWorkspaceSymbolProvider(new TypeScriptWorkspaceSymbolProvider(this.client, allModeIds)));
        ...
        if (diagnostics.length == 0) {
          ...
          let diagnosticList = diagnostics.map((diag2) => {
            ...
            if (!start || !end) {
              ...
              range = import_coc46.Range.create(start.line - 1, start.offset - 1, end.line - 1, end.offset - 1);
            }
    ...
    import_coc46.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
    this.configurationChanged();
  }
